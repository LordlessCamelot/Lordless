 ...
 * ！！！仅支持青龙！！！
 * 
 * 定时配置 0 2-8 * * * 一般跑 5、6 次即可跑满（金币为0），多了没用
 * 
 * 关于是 10W 还是 6W 跟广告有关而不是脚本，想改的话在脚本中搜索 【冲啊】 
 * 
 * 1. 去掉 ks.json
 * 2. 完全不做邀请/助力任务
 * 3. 不需要环境变量，直接将 CK 填在脚本中，方便不同CK做不同管理(比如新号可以跑完就全部自动提现；老号保守些不自动提现)
 * 4. 所有方法增加 try-catch，顺畅到底不断流
 */

// 根据自己信息填好
const ksjsb_guanggao = ""; // 你的广告列表地址
...
const ksjsb_hour = 8; // 提现时间，精确到小时。【建议改掉】
const ksjsb_allMoneyOut = 0; // 是否全部提现，0 不全部提现 1 全部提现。【建议0，小心黑号】
    ...
    for (let Z of p) {
      ...
      if (Z["task"][taskIDCfg["luckydraw"]]["needRun"]) {
        ...
        for (let U = 0; U < Z["task"][taskIDCfg["luckydraw"]]["num"]; U++) {
          // 【### 冲啊】
          // 将下面所有 luckVideoCfg[xxx] 换成你想要的级别就行，不过我不动什么原理，看起来跟配置的参数有关
          // 13 点前和 13 点后看不同的视频获取不同的收益
